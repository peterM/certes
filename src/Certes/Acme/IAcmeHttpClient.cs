...
namespace Certes.Acme
{
    ...
    public interface IAcmeHttpClient
    {
        ...
        Task<AcmeHttpResponse<T>> PostAsync<T>(Uri uri, object payload);
        ...
        /// <param name="uri">The URI.</param>
        /// <returns>The response from ACME server.</returns>
        Task<AcmeHttpResponse<T>> GetAsync<T>(Uri uri);
    }
    ...
    internal static class IAcmeHttpClientExtensions
    {
        ...
        /// </exception>
        internal static async Task<AcmeHttpResponse<T>> PostAsync<T>(this IAcmeHttpClient client,
            ...
            bool ensureSuccessStatusCode) 
        {
            ...
            var payload = await context.SignAsync(entity, location);
            var response = await client.PostAsync<T>(location, payload);
            ...
            while (response.Error?.Status == System.Net.HttpStatusCode.BadRequest &&
                ...
                retryCount-- > 0)
            {
                payload = await context.SignAsync(entity, location);
                response = await client.PostAsync<T>(location, payload);
            }
        ...
        internal static async Task<AcmeHttpResponse<T>> PostAsync<T>(this IAcmeHttpClient client,
            ...
            int retryCount = 1) 
        {
            var payload = jwsSigner.Sign(entity, url: location, nonce: await client.ConsumeNonceAsync());
            var response = await client.PostAsync<T>(location, payload);
            ...
            while (response.Error?.Status == System.Net.HttpStatusCode.BadRequest &&
                ...
                retryCount-- > 0) 
            {
                payload = jwsSigner.Sign(entity, url: location, nonce: await client.ConsumeNonceAsync());
                response = await client.PostAsync<T>(location, payload);
            }
